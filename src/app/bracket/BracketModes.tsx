...
import { ACTIVE_BRACKET_MODES, type BracketMode, type BracketState } from '@/lib/bracket'
...
export default function BracketModes({ userId, pickemState, resetState, tournamentResults }: Props) {
  const [mode, setMode] = useState<BracketMode>(ACTIVE_BRACKET_MODES[0]?.key ?? 'pickem')
  const showToggle = ACTIVE_BRACKET_MODES.length > 1
  const state = mode === 'pickem' ? pickemState : resetState

  return (
    <div>
      {/* Mode toggle — two independent bracket games. Hidden when only one mode is
          active (Bracket Reset is currently gated off via ACTIVE_BRACKET_MODES). */}
      {showToggle && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }} className="wc-noscroll">
          {ACTIVE_BRACKET_MODES.map(m => {
            const on = mode === m.key
            return (
              <button
                key={m.key}
